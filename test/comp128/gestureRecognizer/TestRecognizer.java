...
public class TestRecognizer {
   ...
   public void testPathLength(){
       assertEquals(ORIGINAL_N-1, recognizer.totalPathLength(originalPoints), 0.0001);
...
public void testCentroid() {
    ...
    Deque<Point> testPoint = new ArrayDeque<>();
    testPoint.add(new Point(3.0, 6.0));
    testPoint.add(new Point(2.0, 6.0));
    testPoint.add(new Point(4.0, 7.0));
    testPoint.add(new Point(9.0, 9.0));

    Point centroid = recognizer.centroid(testPoint);

    double expectedX = ((3.0 + 2.0 + 4.0 + 9.0) / 4);
    double expectedY = ((6.0 + 6.0 + 7.0 + 9.0) / 4);

    Point expectedCentroid = new Point(expectedX, expectedY);

    assertEquals(expectedCentroid.getX(), 4.5);
    assertEquals(expectedCentroid.getY(), 7);
}
...
//     */
   ...
   public void testRecognize(){
       ...
       recognize.addTemplate("Arrow", templateGesture);
       recognize.addTemplate("Circle", circleTemplate);


       Deque<Point> testGesture = ioManager.loadGesture("arrowTest.xml");
       Template match = recognize.recognition(testGesture);
       double score = match.getScore();
       assertEquals(0.888684, score, 0.001);

       recognize.addTemplate("test", testGesture);

       Template selfmatch = recognize.recognition(testGesture);
       double selfScore = selfmatch.getScore();

       assertEquals(1.0, selfScore, 0.01); 
   }
}
